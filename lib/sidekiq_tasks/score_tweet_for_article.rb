class ScoreTweetsForArticle
  ...
  def perform(article_id, account_id)
    ...
    article_topics = ArticleDatapoint.first(:article_id => article.id, :provenance => "article_topics").value
    ...
    Score.first_or_create(:article_id => article.id, :ego_id => ego_id, :provenance => "article_tweet_score", :article_created_at => article.created_at)
